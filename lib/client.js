...
//
// Higher-level functions that abstract away some of the protocol details
//

/** Return the current server time as a Date object.
 */
Client.prototype.getDate = function(cb) {
    var p = this.getTime()
        .then(function(time) {
            var date = new Date(
                time.year + 1900, time.month, time.day,

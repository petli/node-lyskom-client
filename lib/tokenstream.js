    ...
    var parser = function(nextBuffer) {
        ...
        while (data) {
            ...
            if ((m = data.match(/^\s*%%(.*)$/m))) {
                ...
                self.emit('error', new ProtocolError(m[1]));
            ...
            // LysKOM
                ...
                if (hs !== 'LysKOM\n'.slice(0, hs.length)) {
                    ...
                    self.emit('error', new ProtocolError('bad server handshake: ' + data));
                    stream.removeAllListeners('data');
                    stream.removeAllListeners('end');
                    buffer = null;
            ...
            // Normal token parsing flow
                ...
                if ((m = data.match(/^\s*([0-9]+)\s/))) {
                    consumed = m[0].length;
                    var tokenLength = m[1].length;

                    token = {
                        type: 'int',
                        raw: buffer.slice(consumed - tokenLength - 1, consumed - 1),
                ...
                // since things seems to be corrupted
                    ...
                    self.emit('error', new ProtocolError('unexpected data from server: ' + data));
                    stream.removeAllListeners('data');
                    stream.removeAllListeners('end');
                    buffer = null;
                    data = '';
    ...
    stream.on('end', function() {
        if (buffer && buffer.length) {
            ...
            if (buffer && buffer.length) {
                ...
                self.emit('error', new ProtocolError('bad data at end of stream: ' + buffer.toString()));
            }
        }
...
TokenStream.ProtocolError = ProtocolError;
 ...
 *
 * - raw:   the raw bytes as a buffer, if type is int (to allow bitstring parsing)
 *
